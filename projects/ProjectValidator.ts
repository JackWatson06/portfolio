import { InvalidValidatorResult, SuccessfulValidatorResult, ValidatorResult } from "./ValidatorResult";
...
export class ProjectValidator implements Validator {  
  validate(project: Project): ValidatorResult {
    const validation_routines: Array<ValidationRoutine> = [
      {
        ...
        is_valid: () => project.media.length != 0 && this.mediaHasOnePicture(project.media)
      },
      {
        error_message: "You can only upload .png, .jpg, .webp, and .mp4 to the media files.",
        is_valid: () => this.mediaHasValidMimeTypes(project.media)
      },
      {
        error_message: "You can only choose 'github', or 'website' for the project links.",
      ...
      {
        error_message: "The primary link you selected does not exist in your links section.",
        is_valid: () => project.live_project_link != undefined &&
        this.linksHaveUrl(
      ...
      },
      {
        ...
        is_valid: () => this.thumbnailMustBePicture(
          project.thumbnail_media,
          project.media,
        )
      }
    ...
    for (const validation_routine of validation_routines) {
      if (!validation_routine.is_valid()) {
        return new InvalidValidatorResult(
          validation_routine.error_message
        );
      }
    }
  ...
  private mediaHasValidMimeTypes(
    project_media_input: Array<MediaElement>,
  ) {
  ...
  private linksHaveValidServiceTypes(
    create_project_links_input: Array<Link>,
  ) {
